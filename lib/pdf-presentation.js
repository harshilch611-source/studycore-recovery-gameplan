...
import { allocatePhaseHours } from './hours';
import { buildRecoveryPresentation as _buildRecoveryPresentation } from './pdf-presentation-recovery';

// Re-export for API route compatibility
export { _buildRecoveryPresentation as buildRecoveryPresentation };
...
function derive(gamePlan, studentData, studentName) {
  ...
  const parsedSum    = parsedPhases.reduce((s, ph) => s + ph.hours, 0);
  const totalHrs     = enteredTotal > 0 ? enteredTotal : (parsedSum > 0 ? parsedSum : 20);

  // Deterministic per-phase hours — override whatever Claude wrote in the titles so the
  // presentation and the game plan PDF show IDENTICAL numbers that always sum to totalHrs.
  // (Shared with pdf-game-plan.js via allocatePhaseHours.)
  const phaseHourAlloc = allocatePhaseHours(parsedPhases.map(p => p.title), totalHrs);
  parsedPhases.forEach((ph, i) => { ph.hours = phaseHourAlloc[i]; });
  ...
  let totalSlides = 6 + phaseCount; // 6 fixed slides + one per phase
