...
export function computeProgram(diagnosticEntries, studentInfo) {
  const { baselineScore, targetScore, sessionFrequency = '2x', blanks = 0 } = studentInfo;
  ...
  for (const topic of topicsWithMisses) {
    ...
    // Safety: don't recommend more than 20 content topics
  ...
  for (let i = 0; i < selectedTopics.length; i++) {
    ...
    const gate3Session = phase2Session + Math.ceil(3 * (sessionFrequency === '2x' ? 2 : 1));
  ...
  const sessionsPerWeek = sessionFrequency === '2x' ? 2 : 1;
  const { plan: weeklyPlan, totalSessions: totalSessionsNeeded } = buildWeeklyPlan(topicSequence, sessionsPerWeek);
  const weeksNeeded = Math.ceil(totalSessionsNeeded / sessionsPerWeek);

  const deliveryCost = lookupDeliveryCost(baselineScore, targetScore);

  // ── Step 8: Program summary ───────────────────────────────────────────────
  const programSummary = {
    topicsToTeach: selectedTopics.length,
    coreSessionsNeeded,
    totalSessionsNeeded,
    sessionsPerWeek,
    weeksNeeded,
