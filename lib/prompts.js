...
export const VISION_EXTRACTION_PROMPT = `Extract ALL score data from this score report. It may be a College Board SAT/PSAT report, an ACT report, or a StudyCore diagnostic / adaptive practice test. Return ONLY valid JSON with no markdown or explanation.

First determine the report type, then extract accordingly.
...
═══ STUDYCORE DIAGNOSTIC / ADAPTIVE TEST REPORTS ═══
If the report shows "StudyCore" branding, an "Adaptive Test" title, an "Accuracy Analysis" chart, or section scores given as RANGES — it is a StudyCore SAT diagnostic. Handle it as follows:
- This is a full SAT simulation. Set "testType": "SAT".
- Scores are given as RANGES (e.g. Total "1100-1140", R/W "490-530", Math "590-630"). For totalScore, rwScore, mathScore use the MIDPOINT of the range rounded to the nearest 10 (1100-1140 → 1120, 490-530 → 510, 590-630 → 610).
- Domain proficiency comes from the "Accuracy Analysis" chart (accuracy % per subject) — read the percentage for each of the 8 subjects. If a percentage is unclear, compute it from that subject's correct ÷ (correct+incorrect+missed) in the unit-wise tables.
- Map the report's subject names to domain keys: "Information and Ideas"→ii, "Craft and Structure"→cs, "Expression of Ideas"→eoi, "Standard English Conventions"→sec, "Algebra"→alg, "Advanced Maths"/"Advanced Math"→am, "Problem Solving and Data Analysis"→psda, "Geometry and Trigonometry"→gt.
- Convert each domain's accuracy % to a band. Use SECTION-SPECIFIC tables (the SAT curve gives more scaled score per accuracy point in Math than in R/W):
  Reading & Writing domains (ii, cs, eoi, sec):
    ≥75% → "610–670" | 65–74% → "550–600" | 58–64% → "500–550" | 52–57% → "490–540" | 45–51% → "450–500" | 38–44% → "400–450" | <38% → "Below 400"
  Math domains (alg, am, psda, gt):
    ≥80% → "680–760" | 72–79% → "610–670" | 64–71% → "550–600" | 57–63% → "500–550" | 50–56% → "490–540" | 42–49% → "450–500" | <42% → "Below 400"
- Put the rich detail into additionalData: the static vs adaptive section breakdown, easy/moderate/hard difficulty counts, per-topic correct/incorrect from the Responses table, time taken, and the score-range caveat note. This is high-value context for the game plan.
...

{
  ...
  "practiceTests": [
    {
      "testNumber": "integer — 1, 2, 3, …",
      "week": "integer — the exact week number this test falls on",
      "targetTotal": "string — e.g. '1290–1320'",
      "targetRW": "string — e.g. '620–640'",
      "targetMath": "string — e.g. '670–690'"
    }
  ],
  "scoreProgression": {
    "intro": "string",
...
- Practice test count and placement is defined in the practiceTests array — see PRACTICE TEST CONSISTENCY below
...
- Finalize the practiceTests array completely before writing any phase content, phaseComplete summaries, or score targets. The array must be complete before any other field references it.
- practiceTests is the single source of truth. No other field may reference a practice test week that does not appear in practiceTests.
- Every week card with a non-null testTarget must correspond exactly to a week integer in practiceTests.
- Every phaseComplete summary that states a practice test count must match the exact count of practiceTests entries whose week falls within that phase's week range.
- Do not reference a practice test in sessionContent, testTarget, or phaseComplete unless that exact week exists in practiceTests.
- Do not assign a full-length practice test as homework in any week card unless that week is listed in practiceTests. This includes any homework item that says "take a full practice test", "full-length SAT", or similar. The early diagnostic test in Week 1 or 2 is the only exception.
- Score range arithmetic: targetTotal upper bound must equal targetRW upper bound + targetMath upper bound. Lower bounds must also sum correctly. Check your arithmetic before writing each practiceTests entry.
- Practice test placement: schedule EXACTLY ONE practice test at the end of EVERY phase — its week number must be the LAST week of that phase's week range — so each phase concludes with a checkpoint that measures progress before the next phase begins. There are no other practice tests.
- Therefore the number of practiceTests entries ALWAYS equals the number of phases, and every phase contains exactly one practice test (in its final week). The final phase's practice test is the last checkpoint before the real exam.
- The early diagnostic in Week 1–2 establishes the baseline and is separate from these phase-end checkpoints; do NOT add it to the practiceTests array.
...
HOUR CONSISTENCY (verify before finalizing):
- Each 1-hour tutoring session = 1 instructional hour. The hours stated in a phase title MUST equal the number of 1-hour sessions described across that phase's week cards. A single-week card = (sessions per week) sessions; a card spanning N weeks (e.g. "Weeks 3–4") = N × (sessions per week) sessions.
- The sum of all phase-title hours MUST equal the total program hours given in the student data. Do not let the phase hours add up to more or less than the program total.
- Before writing each phase title, count the sessions in its week cards and set the hour number to match. Then confirm all phase hours sum to the program total.

HOMEWORK:
- The total homework for each regular week MUST match the "Homework Hrs/Week" value given in the student data. The homework items listed for a regular week should sum to roughly that amount (e.g. if the student data gives 3–4 hours/week, that week's items together total about 3–4 hours).
- Practice test weeks: the full-length practice test plus enough targeted drills to reach the weekly homework target.
- Give each homework item a question count and a time estimate, and make the per-item times add up to the weekly target.
- The programOverview.homework field MUST state the same per-week figure as the student data's "Homework Hrs/Week" — do not invent a different number.
...
export function buildGenerationPrompt(studentData) {
  ...
  const targetTest = targetTestType || testType || 'SAT';
  ...
  "practiceTests": [
    {
      "testNumber": "integer — 1, 2, 3, …",
      "week": "integer — the exact week number this test falls on",
      "targetTotal": "string — ACT composite target e.g. '26'",
      "targetRW": "",
      "targetMath": ""
    }
  ],
...
- Every phaseComplete summary that states a practice test count must match the exact count of practiceTests entries whose week falls within that phase's week range.
- Do not reference a practice test in sessionContent, testTarget, or phaseComplete unless that exact week exists in practiceTests.
- Do not assign a full-length practice test as homework in any week card unless that week is listed in practiceTests. The early diagnostic test is the only exception.
- Practice test placement: schedule EXACTLY ONE practice test at the end of EVERY phase — its week number must be the LAST week of that phase's week range — so each phase concludes with a checkpoint that measures progress before the next phase begins. There are no other practice tests.
- Therefore the number of practiceTests entries ALWAYS equals the number of phases, and every phase contains exactly one practice test (in its final week). The final phase's practice test is the last checkpoint before the real exam.
- The early diagnostic in Week 1–2 establishes the baseline and is separate from these phase-end checkpoints; do NOT add it to the practiceTests array.

HOUR CONSISTENCY (verify before finalizing):
- Each 1-hour tutoring session = 1 instructional hour. The hours stated in a phase title MUST equal the number of 1-hour sessions described across that phase's week cards. A card spanning N weeks = N × (sessions per week) sessions.
- The sum of all phase-title hours MUST equal the total program hours given in the student data — no more, no less.
- Before writing each phase title, count the sessions in its week cards and set the hour number to match; then confirm all phase hours sum to the program total.
...
- Include a question count AND a time estimate in each homework item ("20 English Usage/Mechanics Qs (30 min)"); the items for a regular week must sum to the "Homework Hrs/Week" value given in the student data.
...
/**
 * Recovery Gameplan Generator — for students already enrolled who need a recovery/improvement plan
 */

export const RECOVERY_SYSTEM_PROMPT = `You are StudyCore's recovery gameplan analyst. A student is enrolled in a program (possibly with guarantee) and is not on track to hit their target score by their test date. Your job is to produce a structured recovery plan as JSON.

This document will be shared directly with the student's parents and their assigned tutor. Write all plan content — phases, tutor guidance, parent actions, checkpoints — as if addressing them directly. Be specific, actionable, and clear. No internal jargon.
...
{
  "executiveSummary": "string — 2-3 sentences written for the parent: where the student is, what the gap is, what the plan is to close it",
  "riskLevel": "Green|Yellow|Red",
  "riskFlags": ["string — parent-readable description of the risk"],
  "scoreProgressionAnalysis": {
    "baseline": "number",
    "latestScore": "number or null",
    "targetScore": "number",
    "pointsGained": "number",
    "pointsStillNeeded": "number",
    "projectedFinalScore": "number",
    "onTrack": "true|false",
    "projectionNote": "string — plain-English explanation of the projection for the parent"
  },
  "hoursAnalysis": {
    ...
    "hoursSurplusOrDeficit": "number",
    "sessionPaceNote": "string — explain the hours situation to the parent clearly"
  },
  "domainAnalysis": [
    {
      "domain": "string",
      "baselineBand": "string",
      "currentAccuracy": "string or null",
      "priorityLevel": "Critical|High|Medium|Protect",
      "effortInvested": "string",
      "recommendation": "string — specific instruction for the tutor on what to do in sessions"
    }
  ],
  "engagementAssessment": {
    "assignmentCompletionRate": "string",
    "sessionAttendance": "string",
    "homeworkAdherence": "string",
    "engagementRisk": "None|Low|Medium|High",
    "engagementNote": "string — written for the parent, explaining what engagement looks like and what they can do to support"
  },
  "tutorAssessment": {
    "currentTutor": "string",
    "sessionsWithCurrentTutor": "number",
    "avgSessionDurationHrs": "number or null",
    "tutorConcerns": ["string"],
    "tutorChangeRecommended": "true|false",
    "tutorChangeRationale": "string or null"
  },
  "recoveryPlan": {
    "weeksRemaining": "number",
    "recommendedIntensity": "Maintain|Increase|Significantly Increase",
    "recommendedSessionsPerWeek": "number",
    "recommendedSessionLength": "string",
    "immediateActions": [
      {
        "action": "string — specific, concrete action step",
        "owner": "Tutor|Parent|Both",
        "timeline": "string — e.g. 'This week' or 'Before next session'"
      }
    ],
    "tutorSessionGuidance": "string — 2-3 paragraphs of detailed guidance for the tutor on how to structure sessions, what strategies to use, and what to prioritize given the student's specific weaknesses",
    "parentSupportGuidance": "string — 2-3 paragraphs explaining to the parent what they can do at home to support progress: homework accountability, what to ask their student, how to track progress",
    "phases": [
      {
        "title": "string — e.g. 'RECOVERY PHASE 1 — Domain Triage | Weeks 1–3'",
        "objective": "string — what this phase accomplishes, written for parent and tutor",
        "focusDomains": ["string"],
        "weeklyHours": "number",
        "milestoneTarget": "string — specific score or accuracy target the tutor should hit by end of phase",
        "tutorFocus": "string — 1-2 sentences on exactly what the tutor should be doing in sessions this phase",
        "parentCheckpoint": "string — what the parent should ask or look for by end of this phase"
      }
    ]
  },
  "checkpointTriggers": ["string — plain-English conditions that should prompt a check-in call between parent and StudyCore, e.g. 'If the student misses 2 sessions in a row, contact StudyCore immediately'"],
  "founderNote": "string or null — CONFIDENTIAL: internal note only, does not appear in the PDF sent to parents/tutors",
  "documentTitle": "string"
}

RISK LEVEL DEFINITIONS:
- Green: on track or within 50 pts; hours sufficient; engagement good
- Yellow: 51–150 pts behind pace OR hours deficit <20% OR engagement medium
- Red: 151+ pts behind pace OR hours deficit >20% OR engagement high risk OR tutor change flagged

ANALYTICAL PRINCIPLES:
- 1 hour ≈ 10 points. If hoursRemaining < (pointsStillNeeded / 10), that is a Red flag.
- Domain analysis must cross-reference baseline band with StudyCore platform assignment accuracy. Weak baseline + still weak accuracy = Critical. Weak baseline + improving accuracy = being addressed.
- If practice test scores exist, latestScore is the most recent practice test, not the baseline.
- Do not manufacture data. If a data point is missing, use null and note it in effortInvested.
- Completion rate below 60% is a significant engagement red flag.
- tutorChangeRecommended should only be true if there is a concrete signal: low scores despite many sessions, student notes expressing frustration, or 0 sessions despite >4 weeks.
- founderNote is confidential — does NOT appear in the PDF sent to parents or tutors. Use it for internal concerns only.
- tutorSessionGuidance must be specific to this student's actual weak domains — not generic advice.
- parentSupportGuidance must give concrete actions, not platitudes. Tell parents exactly what to ask and look for.`;

export function buildRecoveryPrompt(data) {
  const {
    studentName,
    grade,
    testType,
  ...
  const sessionHistoryText =
    sessionHistory && sessionHistory.length > 0
      ? sessionHistory
          .slice(-10)
          .map((s) => `  ${s.date} | ${s.tutor} | ${s.durationHrs}hr | ${s.notes || ''}`)
          .join('\n')
      : '  No session history provided.';

  const hiScoresText = hiScoresData
    ? `Overall Completion: ${hiScoresData.overallCompletionPct ?? 'N/A'}%
  Assignments Completed: ${hiScoresData.totalAssignmentsCompleted ?? 'N/A'} / ${hiScoresData.totalAssignmentsGiven ?? 'N/A'}
  Domain Accuracy:
    ${
      Object.entries(hiScoresData.domains || {})
        .map(([k, v]) => `${k}: ${v?.accuracy ?? 'N/A'}% accuracy`)
        .join('\n    ')
    }
  Engagement Flags: ${hiScoresData.engagementFlags?.join(', ') || 'None'}`
    : 'StudyCore platform data not provided.';

  return `STUDENT: ${studentName}, Grade ${grade}
TEST TYPE: ${testType}
BASELINE SCORE: ${baselineScore} (${testType})
  R/W: ${rwScore || 'N/A'} | Math: ${mathScore || 'N/A'}

DOMAIN BANDS (from baseline score report):
  ${domainText}

TARGET SCORE: ${targetScore}
GUARANTEE PROGRAM: ${isGuarantee ? 'YES' : 'NO'}
TARGET TEST DATE: ${targetTestDate || 'Not set'}
WEEKS REMAINING: ${weeksRemaining || 'Calculate from today'}
PROGRAM START DATE: ${programStartDate || 'Unknown'}
TODAY'S DATE: ${new Date().toISOString().split('T')[0]}

HOURS:
  Total Purchased: ${totalHoursPurchased}
  Total Used: ${totalHoursUsed}
  Hours Remaining: ${hoursRemaining}

CURRENT TUTOR: ${currentTutor || 'Not specified'}

SESSION HISTORY (last 10):
${sessionHistoryText}

STUDYCORE PLATFORM / ASSIGNMENT DATA:
${hiScoresText}
...
ADDITIONAL NOTES:
${vaOverrideNotes || 'None'}

Generate the recovery gameplan JSON now. Be clinically honest about achievability. Flag any resource constraint that makes the target unrealistic. Write all parent/tutor-facing content as if you are addressing them directly.`;
}

// ─────────────────────────────────────────────────────────────────────────────
// DIAGNOSTIC-DRIVEN GAMEPLAN GENERATOR
// Used by the new generator that takes per-topic diagnostic data and a
// pre-computed topic sequence from the routing engine.
// Claude's job here is ONLY narrative — all topic priorities and session
// counts have already been computed deterministically.
// ─────────────────────────────────────────────────────────────────────────────

export const GAMEPLAN_SYSTEM_PROMPT = `You are StudyCore's gameplan writer. You receive a student's diagnostic data, a pre-computed topic sequence (already ranked by miss count and priority tier), and a full program summary. Your job is to write the narrative sections of a parent/tutor-facing PDF gameplan.

DO NOT reorder the topics or suggest different ones — the routing engine has already determined the optimal sequence. Your job is to explain WHY this sequence makes sense for this specific student, and give the tutor and parent clear, specific guidance.
...
{
  "executiveSummary": "string — 3-4 sentences. What the diagnostic revealed, what the plan addresses, and what outcome is realistic. Be direct and honest about feasibility.",

  "riskAssessment": "string — 2-3 sentences. Explain what is at risk if the program is not executed as planned. Be specific: reference the score gap, the topic areas, and what happens if compliance slips.",

  "tutorSessionGuidance": "string — 3-4 paragraphs of direct guidance for the tutor. Reference the specific top-priority topics by name. How should they approach Phase 1 for this student? What should they watch for? What are the biggest traps?",

  "parentSupportGuide": "string — 2-3 paragraphs written directly to the parent. What do they need to do at home? What questions should they ask their student after each session? What are the warning signs to watch for?",

  "phaseNarratives": [
    {
      "phaseLabel": "string — e.g. 'Phase 1 — Foundation' or 'Weeks 1–4'",
      "description": "string — 2-3 sentences describing what this phase accomplishes and what success looks like",
      "tutorFocus": "string — 1-2 sentences of specific tutor instruction for this phase",
      "parentCheckpoint": "string — 1-2 sentences: what should the parent verify at the end of this phase?"
    }
  ],

  "checkpointTriggers": [
    "string — plain English: 'If [condition], contact your StudyCore coordinator immediately.'"
  ],

  "bottomLine": "string — 2-3 sentences. The single most important thing this student needs to do to hit the target. Honest, direct, no hedging."
}

Write 3 phaseNarratives maximum. Base them on the topic sequence groupings provided (e.g. first batch of topics = Phase 1, etc.).
Write 5-7 checkpointTriggers.
Address the tutor and parent directly in second person ('you', 'your student').`;

export function buildGameplanPrompt(studentData, routingResult) {
  const { topicSequence, programSummary } = routingResult;
  const {
    studentName, baselineScore, targetScore, rwScore, mathScore,
    grade, currentTutor, programStartDate, targetTestDate,
    sessionFrequency, sessionsPurchased, sessionsCompleted, programNotes,
  } = studentData;

  const topTopics = topicSequence
    .filter(t => !t.isPacing && !t.isWAE)
    .slice(0, 10)
    .map((t, i) => `  ${i + 1}. ${t.topic} (${t.section}, ${t.domain}) — ${t.misses.toFixed(1)} misses, ${t.tierLabel}`)
    .join('\n');

  const feasibilityNote = {
    reachable: 'Diagnostic data supports the target — sufficient misses to convert.',
    tight: 'Target is possible but tight — near-perfect conversion required.',
    unlikely: 'Target is unlikely given the diagnostic data — discuss with the family.',
  }[programSummary.feasibility] || '';

  return `STUDENT: ${studentName}${grade ? `, Grade ${grade}` : ''}
TUTOR: ${currentTutor || 'TBD'}
PROGRAM START: ${programStartDate || 'TBD'}
TEST DATE: ${targetTestDate || 'TBD'}
SESSION FREQUENCY: ${sessionFrequency === '2x' ? '2 sessions/week' : '1 session/week'}

SCORES:
  Diagnostic (baseline): ${baselineScore}
  R/W section: ${rwScore || 'Not provided'}
  Math section: ${mathScore || 'Not provided'}
  Target: ${targetScore}
  Gap to close: ${programSummary.targetGain} points

PROGRAM SUMMARY (computed from diagnostic):
  Topics to teach: ${programSummary.topicsToTeach}
  Core sessions needed: ${programSummary.coreSessionsNeeded}
  Total sessions (incl. shared phases): ${programSummary.totalSessionsNeeded}
  Weeks at current cadence: ${programSummary.weeksNeeded}
  Sessions purchased: ${sessionsPurchased || 'Unknown'}
  Sessions completed: ${sessionsCompleted || '0'}
  Diagnostic misses: ${programSummary.totalDiagnosticMisses}
  Misses that must convert: ${programSummary.missesToConvert}
  Pacing issue (blanks > 5): ${programSummary.needsPacing ? 'YES — pacing work must come first' : 'No'}
  Feasibility: ${programSummary.feasibility.toUpperCase()} — ${feasibilityNote}
  Guarantee status: ${programSummary.guaranteeStatus}

COMPUTED TOPIC SEQUENCE (teach in this order):
${topTopics}
${topicSequence.find(t => t.isWAE) ? '\n  + Wrong Answer Elimination (cross-cutting R&W strategy, ~session ' + topicSequence.find(t => t.isWAE).phase1Session + ')' : ''}
${programSummary.needsPacing ? '\n  NOTE: Pacing, Triage & Test Execution MUST be Session 1 — student left blanks on diagnostic.' : ''}

ADDITIONAL PROGRAM NOTES:
${programNotes || 'None provided.'}

Write the gameplan narrative now. Be specific — reference actual topic names and the score gap. Address the tutor and parent directly. Do not invent topic priorities; they are given above.`;
}
