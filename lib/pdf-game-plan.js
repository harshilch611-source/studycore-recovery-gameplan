...
import { allocatePhaseHours, rewriteTitleHours } from './hours';
import { buildRecoveryPdf as _buildRecoveryPdf } from './pdf-recovery';

// Re-export for API route compatibility
export { _buildRecoveryPdf as buildRecoveryPdf };
...
function buildScoreProgressionRows(practiceTests, studentData) {
  ...
  // Recompute targetTotal from section ranges so the arithmetic is always correct.
  // Claude's upper bounds are sometimes off by 10 (e.g. 650+690=1340 stated as 1330).
  ...
  for (const pt of (practiceTests || [])) {
    const computedTotal = isACT ? null : sumRanges(pt.targetRW, pt.targetMath);
    rows.push({
...
export async function buildGamePlanPdf(data, studentData, studentName) {
  ...
  // Deterministic per-phase hours — identical computation to the presentation builder,
  // so both documents show the same hours and they always sum to total program hours.
  const phases = data.phases || [];
  const alloc  = allocatePhaseHours(phases.map(p => p.title), studentData?.totalHours);
  const phasesWithHours = phases.map((p, i) => ({ ...p, title: rewriteTitleHours(p.title, alloc[i]) }));
